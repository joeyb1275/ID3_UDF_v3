...
#cs #ID3.au3 UDF Latest Changes.......: ;====================================================================
	Release 3.0 Beta1 - 20120501
		...
		-TODO verify compatibility with ID3v2.2
		-TODO Test with other commercial software
		-TODO Code Cleanup
...
Func _ID3ReadTag($Filename, $iTagVersion = 0)
	...
	Switch $iTagVersion
		Case 0 ;All Tags
			...
			$sTAGINFO &= _ID3v2Tag_GetFrameIDs()
...
; Modified.......: 20120501 by joeyb1275
;============================================================================================
Func _ID3GetTagField($sFrameIDRequest, $iFrameID_Index = 1)

	Local $sFrameString

	If StringInStr("|Title|Artist|Album|Year|Comment|Track|Genre|Version1|Speed|Start-Time|End-Time",$sFrameIDRequest,1) Then
		$sFrameString = _ID3v1Field_GetString($sFrameIDRequest)
		SetError(@error)
	Else
		$sFrameString = _ID3v2Frame_GetFields($sFrameIDRequest,$iFrameID_Index)
		SetError(@error)
...
Func _ID3v1Tag_WriteToFile($sFilename)
	...
	;Write MP3 Data
	$hFile = FileOpen($sFilename,16+1) 				;Open for read force binary
	FileSetPos($hFile, $iFileSetPos, $FILE_END)		;Go to End of File (-128 if tag exists)
	FileWrite($hFile,$ID3v1_RawDataBinary)			;Write new tag
	FileClose($hFile)
...
Func _ID3v1Tag_RemoveTag($sFilename)
	...
	If _ID3v1Tag_ReadFromFile($sFilename) <> "" Then
		...
		$hOldFile = FileOpen($sFilename,16+1) 								;Open for read force binary
...
;~ 	MsgBox(0,"$iID3v2_TagSize",$iID3v2_TagSize)
	...
	If _ID3v2Tag_GetHeaderFlags("Unsynchronisation") == 1 Then
		_ID3v2Tag_RemoveUnsynchronisation()
	EndIf
...
;~ 				MsgBox(0,"ZPAD Start",BinaryMid($ID3v2_RawDataBinary,$iBytesRead - $iFrameHeaderLen,20))
...
EndFunc
Func _ID3v2Tag_RemoveUnsynchronisation()
...
Func _ID3v2Tag_GetTagSize()
	...
	Local $byte1 = BitAND(BinaryMid($ID3v2_RawDataBinary,7,1),127)
	Local $byte2 = BitAND(BinaryMid($ID3v2_RawDataBinary,8,1),127)
	Local $byte3 = BitAND(BinaryMid($ID3v2_RawDataBinary,9,1),127)
	Local $byte4 = BitAND(BinaryMid($ID3v2_RawDataBinary,10,1),127)
	...
	Elseif $sFrameID == "TXXX" Then
		...
		If IsString($sDelimiter) Then ;Complex Delimited String
			...
			Switch $aNewFrameStrings[0]
				...
				Case 2
					$bFrameData = _h_ID3v2_CreateFrameTXXX($aNewFrameStrings[1],$aNewFrameStrings[2])
				Case 3
					$bFrameData = _h_ID3v2_CreateFrameTXXX($aNewFrameStrings[1],$aNewFrameStrings[2],Number($aNewFrameStrings[3]))
				Case Else
					;Too many
			EndSwitch
		Else
...
EndFunc
Func _h_ID3v2FrameHeader_GetFlags($bFrameHeaderData)
...
Func _MPEG_GetTagHeader()
	;Check MPEG Header
	Local $bMPEGHeaderCheck = BinaryMid($ID3v2_RawDataBinary,(BinaryLen($ID3v2_RawDataBinary) - 10),50)
	Local $iMPEGHeaderIndex = StringInStr($bMPEGHeaderCheck,"FF")
	Local $sMPEGHeaderHex = StringMid($bMPEGHeaderCheck,$iMPEGHeaderIndex,8)
	If _MPEG_IsValidHeader($sMPEGHeaderHex) Then
		Return $sMPEGHeaderHex
	Else
		Return -1
	EndIf
EndFunc
Func _MPEG_IsValidHeader($MPEGFrameSyncHex)
...
;TODO Clean up APEv2 Code
Func _APEv2Tag_ReadFromFile($Filename)
...
;~ last frame, just before the ID3v1 tag (if any).
	...
	;TODO - Check if ID3v1 is in file and SetExtended and SetError


	Local $APEv2_TAGINFO = "", $APE_TagFound = False
	Local $APE_Version,$APE_TagSize, $sAPE_ItemKeys = ""
	Local $hfile = FileOpen($Filename,16) ;open in binary mode
	FileSetPos($hFile, -(128+32), $FILE_END) ;include 32 bytes to check for APETAG Footer

	$APE_Footer = FileRead($hfile,32)
	$APETAG_ID = BinaryToString(BinaryMid($APE_Footer,1,8))
	$APEv2_TagFrameString = $Filename
	;APE TAG Test Code
...
;~ 		MsgBox(0,"$APETAG",$APETAG_ID & @CRLF & "TagVersion = " & $APE_Version & @CRLF & "TagSize = " & $APE_TagSize)
		FileSetPos($hFile, -(128+$APE_TagSize), $FILE_END)
		$APEv2_RawDataBinary = FileRead($hfile,$APE_TagSize)
		FileClose($hfile)
		$sAPE_ItemKeys = _APEv2_GetItemKeys($APE_TagSize)
	Else
...
EndFunc
Func _APEv2_GetItemKeys($APE_TagSize)
	Local $APE_TAGINFO = ""
	Local $iStartByte = 0,$APE_ItemSize,$APE_ItemKeyterm,$APE_ItemKey
	...
	If $sAPE_ItemKey == -1 Then
		...
		For $istr = 2 to $ataginfo[0]
			...
			If $a2taginfo[0] > 1 Then
				$sAPEv2_ReturnString &= $a2taginfo[1] & " = " & BinaryToString(_APEv2_GetItemValueBinary($a2taginfo[1])) & $StringLineDelimiter
			EndIf
		Next
	Else
		$sAPEv2_ReturnString = BinaryToString(_APEv2_GetItemValueBinary($sAPE_ItemKey))
	EndIf
